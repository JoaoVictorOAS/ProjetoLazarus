...
type
  ...
  TDateTimeHelper = class
    class function ToPG(DT: TDateTime): string;
...
class function TDateTimeHelper.ToPG(DT: TDateTime): string;
begin
  Result := FormatDateTime('yyyy-mm-dd hh:nn:ss', DT);
end;

class function TDateTimeHelper.FromPG(const S: string): TDateTime;
begin
  if S = '' then
  begin
    Result := 0;
...
begin
  ...
  try
    Q.SQL.Text :=
      'SELECT id, tarefa_id, membro_id, tipo, ' +
      'TO_CHAR(inicio,''YYYY-MM-DD HH24:MI:SS'') AS inicio, ' +
      'TO_CHAR(fim,''YYYY-MM-DD HH24:MI:SS'') AS fim, ' +
      'COALESCE(total_min,0) AS total_min, obs ' +
    ...
    begin
      ...
      Lista[Idx].Inicio   := TDateTimeHelper.FromPG(Q.FieldByName('inicio').AsString);
      Lista[Idx].Fim      := TDateTimeHelper.FromPG(Q.FieldByName('fim').AsString);
...
begin
  ...
  try
    ...
    Q.Open;
    if Q.EOF then
      raise Exception.CreateFmt('Registro de hora %d não encontrado', [AId]);
    ...
    Result.Inicio   := TDateTimeHelper.FromPG(Q.FieldByName('inicio').AsString);
    Result.Fim      := TDateTimeHelper.FromPG(Q.FieldByName('fim').AsString);
...
begin
  ...
  try
    Q.SQL.Text :=
      ...
      'VALUES (:tid, :mid, :tipo, :inicio, :fim, :obs) RETURNING id';
    ...
    Q.Params.ParamByName('inicio').AsString := TDateTimeHelper.ToPG(AH.Inicio);
    if AH.Fim > 0 then
      Q.Params.ParamByName('fim').AsString  := TDateTimeHelper.ToPG(AH.Fim)
    else
...
var
  ...
  Status: TTimerStatus;
begin
  Status := TimerAtivo(AMembroId);
  if Status.Ativo then
    PararTimer(Status.RegistroId);
...
begin
  ...
  try
    Q.SQL.Text :=
      'UPDATE horas_tecnicas SET fim = NOW() WHERE id = :id AND fim IS NULL';
