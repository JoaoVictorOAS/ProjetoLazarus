...
begin
  ...
  try
    Q.SQL.Text :=
      ...
      'FROM sprints WHERE projeto_id = :pid';
...
begin
  ...
  try
    Q.SQL.Text :=
      'SELECT id, projeto_id, numero, goal, capacidade_pts, status, ' +
...
begin
  ...
  try
    ...
    Q.Open;
    if Q.EOF then
      raise Exception.CreateFmt('Sprint %d não encontrada', [AId]);
    Result.Id            := Q.FieldByName('id').AsInteger;
    Result.ProjetoId     := Q.FieldByName('projeto_id').AsInteger;
    Result.Numero        := Q.FieldByName('numero').AsInteger;
    Result.Goal          := Q.FieldByName('goal').AsString;
...
begin
  ...
  try
    Q.SQL.Text :=
      ...
      'VALUES (:pid, :num, :goal, :cap, :inicio::DATE, :fim::DATE) RETURNING id';
...
begin
  ...
  try
    Q.SQL.Text :=
      'UPDATE sprints SET status=''ativa'', data_inicio=CURRENT_DATE WHERE id=:id';
    Q.Params.ParamByName('id').AsInteger := AId;
    Q.ExecSQL;
    TDBConnection.Commit;
  finally
...
begin
  ...
  try
    Q.SQL.Text :=
      'UPDATE sprints SET status=''encerrada'', data_fim=CURRENT_DATE WHERE id=:id';
    Q.Params.ParamByName('id').AsInteger := AId;
    Q.ExecSQL;
    TDBConnection.Commit;
  finally
