...
begin
  ...
  try
    Q.SQL.Text :=
      ...
      'VALUES (:sid, :mid, :titulo, :desc, :tipo, :status, :pts, :prio) RETURNING id';
...
begin
  ...
  try
    Q.SQL.Text :=
      ...
      'pontos=:pts, prioridade=:prio, membro_id=:mid, atualizado_em=NOW() WHERE id=:id';
    Q.Params.ParamByName('titulo').AsString := AT.Titulo;
    Q.Params.ParamByName('desc').AsString   := AT.Descricao;
    Q.Params.ParamByName('tipo').AsString   := AT.Tipo;
    Q.Params.ParamByName('status').AsString := AT.Status;
...
begin
  ...
  try
    Q.SQL.Text :=
      'UPDATE tarefas SET status=:status, atualizado_em=NOW() WHERE id=:id';
